...
object Declassify {
  def apply(expr: Data, lbl: Label) = expr match {
    case (e: Element) => new DeclassifyE(e, lbl)
    case _ => new Declassify(expr, lbl)
  }
}

class Declassify(val expr: Data, override val lbl: Label) extends Data {
  // ref controls the translation to FIRRTL
  override def ref = Declass(expr.ref, lbl)
  def allElements = expr.allElements
  def cloneTypeWidth(w: Width) : this.type =
    Declassify(expr.cloneTypeWidth(w), lbl).asInstanceOf[this.type]
  def toType = expr.toType
  def width = expr.width
  def legacyConnect(that: Data)(implicit sourceInfo: SourceInfo) =
    expr.legacyConnect(that)(sourceInfo)
  def cloneType : this.type =
    Declassify(expr.cloneType, lbl).asInstanceOf[this.type]
  def flatten = expr.flatten // TODO this seems like it might cause problems
  def toPrintable = expr.toPrintable 
  def do_fromBits(that: Bits)(implicit sourceInfo: SourceInfo,
    compileOptions: CompileOptions): this.type =
      Declassify(expr.do_fromBits(that)(sourceInfo, compileOptions),
        lbl).asInstanceOf[this.type]
}

class DeclassifyE(val expr: Element, override val lbl: Label) extends Element(expr.width) {
  // ref controls the translation to FIRRTL
  override def ref = Declass(expr.ref, lbl)
  // this should behave exactly the same as expr, except when this is 
  // referenced in a pushCommand it pushes the declassified expr.
  def cloneTypeWidth(w: Width) : this.type =
    Declassify(expr.cloneTypeWidth(w), lbl).asInstanceOf[this.type]
  def toType = expr.toType
  def cloneType : this.type =
  ...
  def do_fromBits(that: Bits)(implicit sourceInfo: SourceInfo,
    compileOptions: CompileOptions): this.type =
      Declassify(expr.do_fromBits(that)(sourceInfo, compileOptions),
        lbl).asInstanceOf[this.type]
  override private[core] def binding_=(target: Binding): Unit =
    expr.binding_=(target)
  override private[core] def binding = expr.binding
  override def allElements = expr.allElements
  override def dir = expr.dir
  override def name = expr.name
  override def widthKnown = expr.widthKnown
}
...
class Endorse(val expr: Data, override val lbl: Label) extends Data {
  // ref controls the translation to FIRRTL
  override def ref = Endor(expr.ref, lbl)
  def allElements = expr.allElements
  def cloneTypeWidth(w: Width) : this.type =
    Endorse(expr.cloneTypeWidth(w), lbl).asInstanceOf[this.type]
  def toType = expr.toType
  def width = expr.width
  def legacyConnect(that: Data)(implicit sourceInfo: SourceInfo) =
    expr.legacyConnect(that)(sourceInfo)
  def cloneType : this.type =
    Endorse(expr.cloneType, lbl).asInstanceOf[this.type]
  def flatten = expr.flatten // TODO this seems like it might cause problems
  def toPrintable = expr.toPrintable
  def do_fromBits(that: Bits)(implicit sourceInfo: SourceInfo,
    compileOptions: CompileOptions): this.type =
      Endorse(expr.do_fromBits(that)(sourceInfo, compileOptions),
        lbl).asInstanceOf[this.type]
}

class EndorseE(val expr: Element, override val lbl: Label) extends Element(expr.width){
  // ref controls the translation to FIRRTL
  override def ref = Endor(expr.ref, lbl)
  def cloneTypeWidth(w: Width) : this.type =
    Endorse(expr.cloneTypeWidth(w), lbl).asInstanceOf[this.type]
...
sealed class Clock extends Element(Width(1)) {
  ...
  // TODO: assuming this is the bottom type. It would be better to figure 
  override def lbl = Label(Level("L"), Level("H"))
